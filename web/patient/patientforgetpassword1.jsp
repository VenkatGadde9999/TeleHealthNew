...
PreparedStatement pstmt = con1.prepareStatement("update patientlogin set status=? where patientid=? and firstname = ? and lastname = ?");
...
if(x>0)
{
    ...
    out.println("<html><script>alert('Activation code send to your email');"
            + "window.location=\'patient/forgetpasswordactivate.jsp?userid="+email+"\';</script></html> ");
...
//response.sendRedirect("alert('Invalid User id/password')");
    out.println("<html><script>alert('Forget password Activation failed/Invalid emailid or first name or lastname');"
               + "window.location='patientforgetpassword.jsp';</script></html> ");
}
}
catch(Exception e2)
{
