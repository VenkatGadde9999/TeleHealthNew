...
<%
    ...
    String status = "Appointment Rejected";
    ...
    try {
        ...
        if (n > 0) {
            ...
            out.println("<html><script>alert('Applointment Request Rejected successfully');"
...
//response.sendRedirect("alert('Invalid User id/password')");
            out.println("<html><script>alert('Appointment Request Rejected process failed');"
                    + "window.location='manageappointment.jsp';</script></html> ");
        }

    } catch (Exception e1) {

        out.println("<html><script>alert(\"Appointment Request Rejection process Failed " + e1 + "\");"
                + "window.location='manageappointment.jsp';</script></html> ");
    } finally {
        try {
            con1.close();
